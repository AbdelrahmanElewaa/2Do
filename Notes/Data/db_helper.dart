...
class DBHelper {
  ...
  final createSQL = '''CREATE TABLE $kTableName (
      ...
      ${Columns.title.name} TEXT, 
      ${Columns.content.name} TEXT,
  ...
  Future<List<Map<String, dynamic>>> queryAllRowsAsc() async {
    Database db = await database;
    return await db.query(
      _table,
      orderBy: 'date ASC',
  ...
  Future<List<Map<String, dynamic>>> queryRows(title) async {
    Database db = await database;
    return db.query(_table, where: "${Columns.title.name} LIKE '%$title%'");
  }
